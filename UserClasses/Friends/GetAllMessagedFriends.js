  ...
  async get() {
    try {
      const allFriends = await FriendsModel.findOne(
        { userId: this.userId },
        { messagedFriends: 1, _id: 0 }
      );

      const pages = caculatePages(
        this.pageSize,
        allFriends.messagedFriends.friendsList.length
      );
      ...
      if (this.pageNumber > pages) {
        if (pages) {
          ...
          return {
            error: true,
            information: "You have no messaged friends",
          };
        }
      }
      ...
      const resultArray = allFriends.messagedFriends.friendsList.slice(
        start,
        end
      );
...
module.exports = GetAllMessagedFriends;
